...
//  - JohoSpecialDayEditorSheet: Holiday/Observance editor
//  - JohoAddSpecialDaySheet: Entry type menu
//  - JohoEventEditorSheet: Countdown event editor
//
...
struct JohoEventEditorSheet: View {
    ...
    private var selectedDate: Date {
        calendar.date(from: DateComponents(year: selectedYear, month: selectedMonth, day: selectedDay)) ?? Date()
    }
    ...
    init(event: CountdownEvent, onSave: @escaping (String, Date, String, String) -> Void) {
        ...
        _selectedYear = State(initialValue: Calendar.current.component(.year, from: event.targetDate))
        _selectedMonth = State(initialValue: Calendar.current.component(.month, from: event.targetDate))
        _selectedDay = State(initialValue: Calendar.current.component(.day, from: event.targetDate))
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 0) {
                ...
                .frame(height: 56)
                .background(eventAccentColor.opacity(0.7))

                Rectangle().fill(JohoColors.black).frame(height: 1.5)

                // Name row
                HStack(spacing: 0) {
                    Circle().fill(eventAccentColor).frame(width: 10, height: 10)
                        .overlay(Circle().stroke(JohoColors.black, lineWidth: 1.5))
                        .frame(width: 40).frame(maxHeight: .infinity)
    ...
    private func monthName(_ month: Int) -> String {
        let formatter = DateFormatter()
        formatter.dateFormat = "MMM"
        return formatter.string(from: calendar.date(from: DateComponents(year: 2024, month: month, day: 1)) ?? Date())
    }
